...
#           print self.ip_mac_port                  
	...
	ofproto = datapath.ofproto
	   ...
	   self.total_load += 1
           ...
           ##VERIFICATION
           # if self.total_load == 30000:
           #   sys.stdout.write("30000 REACHED\n")
...
#                 load += self.server_load[i]
              ...
              if self.total_load != 0:
                for i in range (0, self.maxID + 1):
                   sys.stdout.write(str((self.server_load[i]*1.0/self.total_load)*100) + "%\t")
...
#          print values
    ...
    def flow_removal_handler(self, ev):
        ...
        dpid = dp.id
        serverID = msg.cookie
        print "Client released serverID = ", msg.cookie
        if serverID < len(self.server_load):
